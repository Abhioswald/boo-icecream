...
export default function OrderModal({ isOpen, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 bg-black/90 backdrop-blur-xl animate-in fade-in duration-300">
      <div
        className="relative w-full max-w-md rounded-3xl glass-card border border-white/10 p-6 sm:p-8 bg-[#0a0a0f] shadow-[0_25px_80px_rgba(0,0,0,0.95)] text-white overflow-hidden"
      ...
      >
        ...
        <button
          ...
          className="absolute top-6 right-6 p-2 rounded-full bg-zinc-900/80 hover:bg-zinc-800 text-zinc-400 hover:text-white transition-colors cursor-pointer"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        {isSubmitted ? (
          <div className="py-8 text-center space-y-4">
            ...
            <p className="text-zinc-400 text-xs tracking-wider max-w-xs mx-auto">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full bg-zinc-900/80 border border-zinc-800 rounded-xl px-4 py-3 text-sm text-white placeholder-zinc-600 focus:outline-none focus:border-purple-500 font-sans"
              />
            </div>

            <button
              type="submit"
              className="w-full py-3.5 bg-gradient-to-r from-purple-600 via-fuchsia-600 to-purple-600 hover:from-purple-500 hover:to-fuchsia-500 text-white font-display font-black text-xs tracking-[0.25em] uppercase rounded-full transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] shadow-[0_0_25px_rgba(168,85,247,0.3)] flex items-center justify-center gap-2 cursor-pointer"
            >
